...
checks them against ransomware leak sites, onion pages fetched over Tor, and breach and paste
data. It scores each hit by what surrounds it, remembers what it has already reported, and
writes a report that says what to do about every hit. New hits trigger a Windows notification,
and optionally a phone push, a webhook or an email. A scheduled task runs it daily.

It only reads. It never logs in, posts, buys, or downloads files, and it keeps no page bodies.
It stores only the URL, the times, and a short snippet around each match.
